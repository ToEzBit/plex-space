...
- A persistent Command center lets the user see which Spaces are open, switch between them, and later expose Space-scoped controls without leaving the active Layout.
...
- Terminal real estate is reduced by 200 px horizontally while the Command center is open. Accepted: the toggle provides an escape hatch when the user needs full width.
